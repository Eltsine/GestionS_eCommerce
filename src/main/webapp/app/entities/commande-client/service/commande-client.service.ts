...
export class CommandeClientService {
  ...
  protected convertDateFromClient(commandeClient: ICommandeClient): ICommandeClient {
    return Object.assign({}, commandeClient, {
      dateCommande: commandeClient.dateCommande?.isValid() ? commandeClient.dateCommande.toJSON() : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((commandeClient: ICommandeClient) => {
        commandeClient.dateCommande = commandeClient.dateCommande ? dayjs(commandeClient.dateCommande) : undefined;
      });
    }
    return res;
  }
